...
> Usage: /cadence:loop-watch <id>
> Example: /cadence:loop-watch L-1721000000
...
The state file must exist at `<projectRoot>/cadence/loops/<id>/state.json`.

If the file does not exist, tell the user:

> No loop state found for `<id>`.
> Run `/cadence:loop-start` to begin a new loop, or check that the id is correct.
