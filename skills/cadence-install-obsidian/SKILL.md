...
- The script never overwrites an existing cadence/.obsidian/ — treat reason "already-exists" as success ("vault already configured"), not an error. Re-running this command is always safe.
</important>
...
5. Run `scaffold` regardless of the outcome of steps 2–4 — it only writes files and doesn't require Obsidian to be installed or running. Report the result:
   - `created` — vault config created at `cadence/.obsidian/`.
   - `already-exists` — vault already configured; untouched.
   - `no-cadence-dir` — there's no `cadence/` here yet: suggest running `/cadence:refine` (or another cadence command) first so there's a brain worth opening; this command never creates `cadence/` itself.
6. If Obsidian is installed and the vault is configured, tell the user to open the project's `cadence/` folder in Obsidian ("Open folder as vault") to browse the brain graph.
...
- **No `cadence/` directory:** redirect to `/cadence:refine`; do not create `cadence/`.
- **`cadence/.obsidian/` already exists:** success ("vault already configured"), not an error.
