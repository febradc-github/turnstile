...
11. Dispatch the `brain-curator` agent with: the source files this pass created or changed (path, one-line purpose, exports, and known imports/callers -- the coder's report has these) so their `cadence/code/` notes are created or updated, plus anything worth remembering (an architectural decision, a gotcha, prior work this ticket built on -- including anything the `cadence-coder` report flagged under Notes). This dispatch always fires when code changed; it is skipped only when nothing was implemented.
